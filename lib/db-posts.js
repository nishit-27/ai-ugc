...
export async function createPost({ jobId, accountId, lateAccountId, caption, videoUrl, platform, status, scheduledFor, latePostId, platformPostUrl, createdBy, apiKeyIndex = 0 }) {
  const normalizedScheduledFor = coerceTimestampValue(scheduledFor, 'scheduledFor');
  const result = await rawSql`
    ...
    VALUES (${jobId || null}, ${accountId || null}, ${lateAccountId || null}, ${caption || null}, ${videoUrl || null}, ${platform || 'tiktok'}, ${status || 'draft'}, ${normalizedScheduledFor}, ${latePostId || null}, ${platformPostUrl || null}, ${createdBy || null}, ${apiKeyIndex})
...
export async function updatePost(id, updates) {
  ...
  const normalizedPublishedAt = coerceTimestampValue(publishedAt, 'publishedAt');
  const normalizedLastCheckedAt = coerceTimestampValue(lastCheckedAt, 'lastCheckedAt');

  const result = await rawSql`
    UPDATE posts SET
      status = COALESCE(${status || null}, status),
      published_at = COALESCE(${normalizedPublishedAt}, published_at),
      ...
      last_checked_at = COALESCE(${normalizedLastCheckedAt}, last_checked_at),
      updated_at = NOW()
    WHERE id = ${id}
    RETURNING *
  `;
