...
async function _initDatabaseImpl() {
  ...
  await sql`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS fal_recovery_token TEXT`;
  ...
  await sql`ALTER TABLE template_jobs ADD COLUMN IF NOT EXISTS source_trim_start REAL`;
  await sql`ALTER TABLE template_jobs ADD COLUMN IF NOT EXISTS source_trim_end REAL`;
  await sql`ALTER TABLE template_jobs ADD COLUMN IF NOT EXISTS fal_recovery_token TEXT`;
  await sql`ALTER TABLE template_jobs ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW()`;
  ...
  // Stuck-job recovery + post-status filtering hit these columns on every cron tick.
  await sql`CREATE INDEX IF NOT EXISTS idx_template_jobs_status_updated_at ON template_jobs(status, updated_at)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_template_jobs_post_status ON template_jobs(post_status) WHERE post_status IS NOT NULL`;
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_agent_chats_updated_at ON agent_chats(updated_at DESC)`;
  ...
  `;
  await sql`ALTER TABLE agent_chat_messages ADD COLUMN IF NOT EXISTS created_by TEXT`;
  await sql`CREATE INDEX IF NOT EXISTS idx_agent_chat_messages_chat_id ON agent_chat_messages(chat_id, created_at)`;
  await sql`ALTER TABLE agent_chat_messages ADD COLUMN IF NOT EXISTS created_by_name TEXT`;
  await sql`ALTER TABLE agent_chat_messages ADD COLUMN IF NOT EXISTS created_by_image TEXT`;

  // ── Late API key index per profile ──
  await sql`
  ...
  // ── Learned per-key profile limits ──
  await sql`
  ...
  // ── Twitter pipelines ──
  await sql`
    CREATE TABLE IF NOT EXISTS twitter_pipelines (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      name TEXT NOT NULL,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_twitter_pipelines_status ON twitter_pipelines(status)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_twitter_pipelines_created_at ON twitter_pipelines(created_at DESC)`;
  // Added after initial release: selected models, publish mode, and per-step
  // execution results so the pipeline can resolve accounts and be audited in /jobs.
  await sql`ALTER TABLE twitter_pipelines ADD COLUMN IF NOT EXISTS model_ids JSONB NOT NULL DEFAULT '[]'::jsonb`;
  await sql`ALTER TABLE twitter_pipelines ADD COLUMN IF NOT EXISTS publish_mode TEXT NOT NULL DEFAULT 'now'`;
  await sql`ALTER TABLE twitter_pipelines ADD COLUMN IF NOT EXISTS results JSONB NOT NULL DEFAULT '[]'::jsonb`;
