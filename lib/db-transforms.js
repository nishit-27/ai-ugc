...
function parsePipelineValue(pipeline) {
...
export function transformTemplateJob(row) {
  if (!row) return null;
  const pipeline = parsePipelineValue(row.pipeline);
  const rawStepResults = typeof row.step_results === 'string' ? JSON.parse(row.step_results) : (row.step_results || []);
  const stepResults = normalizeTemplateJobStepResults(pipeline, fixStepResultUrls(rawStepResults));
  return {
    id: row.id,
    name: row.name,
    status: row.status,
