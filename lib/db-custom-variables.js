...
export async function setJobVariableValues(templateJobId, values) {
  ...
  const rows = values.map(({ variableId, value }) => ({
    templateJobId,
    variableId,
    value,
  }));
  // ON CONFLICT DO UPDATE on a composite key — Drizzle's bulk insert supports this.
  return db.insert(jobVariableValues).values(rows).onConflictDoUpdate({
...
export async function syncJobVariablesToMedia(jobId, linkedMediaItems) {
  if (!linkedMediaItems || linkedMediaItems.length === 0) return;
  const mediaIds = linkedMediaItems.map((m) => m.id);
  // Single batched delete instead of one per media item.
  await db.delete(mediaVariableValues).where(inArray(mediaVariableValues.mediaItemId, mediaIds));
  // Single batched insert: cross-join the job's variable rows against all media ids.
  await rawSql`
    INSERT INTO media_variable_values (media_item_id, variable_id, value)
    SELECT m.media_item_id, jvv.variable_id, jvv.value
    FROM job_variable_values jvv
    CROSS JOIN unnest(${mediaIds}::uuid[]) AS m(media_item_id)
    WHERE jvv.template_job_id = ${jobId}
    ON CONFLICT (media_item_id, variable_id) DO NOTHING
  `;
}
