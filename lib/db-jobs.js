...
function applyUrlFix(row) {
  ...
  return { ...safeRow, outputUrl: fixOutputUrl(safeRow.outputUrl) };
}
...
export async function updateJob(id, updates) {
  const { status, step, outputUrl, error, completedAt, falRequestId, falEndpoint, falRecoveryToken, videoUrl, videoSource } = updates;
  ...
  if (hasKey('falRecoveryToken')) setObj.falRecoveryToken = falRecoveryToken || null;
...
export async function getStuckJobs(minutesThreshold = 10) {
  const result = await db.select().from(jobs).where(
    and(
      ...
      lt(jobs.updatedAt, sql`NOW() - INTERVAL '1 minute' * ${minutesThreshold}`)
    )
...
export async function getJobByRecoveryToken(jobId, recoveryToken) {
  const result = await db.select().from(jobs).where(
    and(
      eq(jobs.id, jobId),
      eq(jobs.falRecoveryToken, recoveryToken),
    )
  ).limit(1);
  return result[0] ? applyUrlFix(result[0]) : null;
}
