...
export async function acquireTemplateJobPostLock(id, staleMinutes = 3) {
  await sql`
    DELETE FROM template_job_post_locks
    WHERE locked_at < NOW() - (${staleMinutes} * INTERVAL '1 minute')
  `;

  const result = await sql`
    INSERT INTO template_job_post_locks (job_id, locked_at)
    VALUES (${id}, NOW())
...
export async function releaseTemplateJobPostLock(id) {
  await sql`DELETE FROM template_job_post_locks WHERE job_id = ${id}`;
}
...
export async function getCompletedTemplateJobVideos() {
  const result = await sql`
    SELECT id, output_url, created_at, completed_at, created_by, model_id
    FROM template_jobs
