...
export async function getAllTemplateJobs({ limit = 500 } = {}) {
  // Cap the unbounded list so the response stays under Vercel's body limit
  // even after tens of thousands of jobs accumulate. Older jobs are still
  // reachable via /api/templates/[id]. Default chosen to comfortably cover
  // a few months of activity for the dashboard's polling view.
  let q = db.select().from(templateJobs).orderBy(desc(templateJobs.createdAt));
...
/** @param {string[] | null} [modelIds] */
export async function getTemplateJobsForLibraryPicker(modelIds = null) {
  const baseSelect = {
    id: templateJobs.id,
    modelId: templateJobs.modelId,
    pipelineBatchId: templateJobs.pipelineBatchId,
    status: templateJobs.status,
    outputUrl: templateJobs.outputUrl,
    stepResults: templateJobs.stepResults,
    createdAt: templateJobs.createdAt,
    completedAt: templateJobs.completedAt,
  };

  const filters = [
    eq(templateJobs.status, 'completed'),
    isNotNull(templateJobs.modelId),
  ];
  if (Array.isArray(modelIds) && modelIds.length > 0) {
    filters.push(inArray(templateJobs.modelId, modelIds));
  }
