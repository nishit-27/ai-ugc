...
export async function getGeneratedImagesPage(limit, offset) {
  const [rows, countResult] = await Promise.all([
    sql`SELECT * FROM generated_images ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`,
    sql`SELECT COUNT(*)::int AS total FROM generated_images`,
  ]);
