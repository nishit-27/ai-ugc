...
async function _initDatabaseImpl() {
  ...
  // Partial indexes for webhook lookups by fal_request_id
  ...
  // Model-to-social-account mappings (for Master Canvas)
  await sql`
  ...
  // Master batch columns on pipeline_batches
  ...
  // Track who created each record (Google first name from session)
...
// Jobs CRUD
export async function createJob({ tiktokUrl, videoUrl, videoSource = 'tiktok', imageUrl, customPrompt, maxSeconds, batchId, status = 'processing', step = 'Starting...', createdBy }) {
...
// Update template job post status (for master batch review)
...
// Template Jobs CRUD
export async function createTemplateJob({ name, pipeline, videoSource, tiktokUrl, videoUrl, pipelineBatchId, modelId, regeneratedFrom = null, createdBy }) {
...
export async function updateTemplateJob(id, updates) {
  ...
  // Use explicit null check — 'outputUrl' key present with null value means clear the field
...
export async function getGeneratedImagesPage(limit, offset, options = {}) {
  ...
  const rows = await sql`SELECT * FROM generated_images ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
  if (!includeTotal) {
    return { images: rows.map(transformGeneratedImage), total: null };
  }
  const countResult = await sql`SELECT COUNT(*)::int AS total FROM generated_images`;
  return { images: rows.map(transformGeneratedImage), total: countResult[0].total };
}

export async function getGeneratedImagesCount() {
  const result = await sql`SELECT COUNT(*)::int AS total FROM generated_images`;
...
function transformModelAccountMapping(row) {
  if (!row) return null;
  return {
    id: row.id,
    modelId: row.model_id,
...
/**
 * Look up a regular job by its FAL request_id (for webhook delivery).
 */
...
/**
 * Look up a template job by its FAL request_id (for webhook delivery).
 */
