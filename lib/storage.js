...
import { isRetryableError, retry } from './retry.ts';
...
export async function downloadToBuffer(url) {
  ...
  return retry(async () => {
    ...
    onRetry: (error, attempt, delayMs) => {
      ...
      console.warn(`[Storage] downloadToBuffer failed for ${url} (attempt ${attempt}), retrying in ${delayMs}ms: ${message}`);
    },
  });
}
...
export async function downloadToPath(url, localPath) {
  ...
  return retry(async () => {
    ...
    onRetry: (error, attempt, delayMs) => {
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`[Storage] downloadToPath failed for ${url} (attempt ${attempt}), retrying in ${delayMs}ms: ${message}`);
    },
  });
}
