...
export async function createGenerationRequest({
  ...
  const rows = await sql`
    INSERT INTO generation_requests (type, provider, model, status, cost, duration_seconds, error, metadata, created_by, created_by_email)
    VALUES (${type}, ${provider}, ${model}, ${status}, ${cost}, ${durationSeconds}, ${error}, ${metadata ? JSON.stringify(metadata) : null}, ${createdBy}, ${createdByEmail})
    RETURNING *
  `;
  return rows[0];
}
