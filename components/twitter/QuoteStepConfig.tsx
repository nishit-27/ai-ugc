...
interface QuoteStepConfigProps {
  ...
  onChange: (config: Partial<TwitterQuoteConfig>) => void;
...
export default function QuoteStepConfig({ config, onChange, onGenerate, onFetchTweet, isGenerating }: QuoteStepConfigProps) {
  ...
  const handleFetchTweet = useCallback(async () => {
    ...
    if (tweet && typeof tweet === 'object' && 'tweet_id' in (tweet as Record<string, unknown>)) {
      const t = tweet as TwitterQuoteConfig['targetTweet'] & { tweet_id: string };
  ...
  const handleEnhance = async () => {
    const result = await onGenerate({ mode: 'enhance', currentContent: config.content, type: 'quote' });
  ...
  return (
    <div className="space-y-4">
      {/* Tweet URL */}
      <div>
        <label className="mb-1.5 block text-xs font-medium text-[var(--text-muted)]">Tweet to quote</label>
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-primary)] py-2 pl-9 pr-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[#FFAD1F] focus:outline-none"
            />
          </div>
          <button
      ...
      {config.targetTweet && (
        <div className="rounded-xl border border-[#FFAD1F]/20 bg-[#FFAD1F]/5 p-4">
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="text-xs font-medium text-[var(--text-muted)]">Your Commentary</label>
        ...
        <textarea
          ...
          rows={3}
          className="w-full resize-none rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[#FFAD1F] focus:outline-none"
        />
      </div>
      ...
      <div className="flex gap-2">
        ...
        {config.content && (
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-[var(--border)] px-3 py-2 text-sm text-[var(--text-secondary)] hover:border-[var(--primary)] hover:text-[var(--primary)] disabled:opacity-50"
          >
