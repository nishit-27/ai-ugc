  ...
  if (batches.length === 0) {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-12 text-center">
        <Crown className="mx-auto mb-3 h-8 w-8 text-[var(--text-muted)]" />
        <h3 className="mb-2 font-semibold">No master batches yet</h3>
        <p className="mb-4 text-sm text-[var(--text-muted)]">Switch to Master mode in Pipelines to run across multiple models</p>
        <Link
          href="/templates"
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {paginatedItems.map((batch) => {
          ...
          const modelCount = batch.masterConfig?.models?.length || 0;

          return (
            <Link
              key={batch.id}
              href={`/jobs/master/${batch.id}`}
              className={`group relative overflow-hidden rounded-xl border transition-all hover:shadow-md ${
                isActive ? 'border-purple-300 dark:border-purple-800' :
            ...
            >
              {isActive && (
                <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-purple-400 via-purple-500 to-purple-400 animate-pulse" />
              )}
                ...
                <div className="mb-3 flex items-start justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-sm font-bold ${
                      isActive ? 'bg-purple-100 text-purple-600 dark:bg-purple-950/40 dark:text-purple-400' :
                  ...
                  <span className={`shrink-0 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                    ...
                    isActive ? 'bg-purple-50 text-purple-600 dark:bg-purple-950/40 dark:text-purple-400' :
                    'bg-[var(--accent)] text-[var(--text-muted)]'
                  }`}>
              ...
              <div className="flex items-center justify-end border-t border-[var(--border)] bg-[var(--background)] px-3 py-1.5 text-[10px] text-[var(--text-muted)] opacity-0 transition-opacity group-hover:opacity-100">
                Review videos <Arrow className="ml-0.5 h-3 w-3" />
              </div>
            </Link>
          );
        })}
